...
// AI Elements' audio player, adapted to this stack. Two changes from the registry copy:
//
//   - It composes with Radix's `asChild`; our Button and ButtonGroupText are base-ui, which
//     composes with `render`. base-ui also assumes the rendered element is a real <button>
//     and warns when it is not — media-chrome's controls are custom elements, so they say so
//     with nativeButton={false}.
//   - The control bar is forced LTR. A media timeline is not text: it is the tape, and it runs
//     left to right in every locale. Material Design 3 states it outright — "media controls for
//     video or audio players are always LTR" — and YouTube, Spotify and the rest do not mirror
//     a scrubber in Arabic or Persian. Left in RTL, the fill grows backwards and the elapsed
//     and total times swap places. Same class of bug as KaTeX inheriting `direction: rtl`.
...
export const AudioPlayerControlBar = ({
	...
	<MediaControlBar data-slot="audio-player-control-bar" dir="ltr" {...props}>
